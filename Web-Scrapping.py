...
def DCGroupSubmission (scrapywebsite):
...
#                finalxx=finalxx+x+'\t'
        ...
        try:
            ...
            for link in links:       
                x = re.findall(".*CONTACT*.",link.text.upper())
                if str(x) != 'None':
                    linkx=link.get("href")
            ...
            for contactLink in contactLinks:
                ...
                #z = re.findall(r'[\w\.-]+@[\w\.-]+', contactLink.text)
                #zzemails = re.findall(r"[a-z0-9\.\-+]+@[a-z0-9\.\-+]+[a-z]{2,3}",contactLink.text)
                #re.findall
                if str(z) != 'None':
                    finalemail=contactLink.get("href").replace("mailto:","")
                
                #if zzemails!='None':
                 #   finalemail = finalemail +','+contactLink.get("href")
...
dcresults = 'I:\CBA\Term1\DC\DCRESULTS_v3.csv'
htmldir = 'I:\CBA\Term1\DC\\assignmentData'
dcstring = ""
...
dcf.write(headers)
fpath = 'I:\CBA\Term1\DC\\assignmentData\\1.html'
dcstring = DCGroupSubmission (fpath)
dcf.write(dcstring)
